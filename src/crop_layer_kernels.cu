...
void forward_crop_layer_gpu(crop_layer l, network net)
{
    ...
    float radians = l.angle*3.14159265f/180.f;
    ...
    levels_image_kernel<<<cuda_gridsize(size), BLOCK>>>(net.input_gpu, l.rand_gpu, l.batch, l.w, l.h, net.train, l.saturation, l.exposure, translate, scale, l.shift);
    check_error(cudaPeekAtLastError());

    size = l.batch*l.c*l.out_w*l.out_h;

    forward_crop_layer_kernel<<<cuda_gridsize(size), BLOCK>>>(net.input_gpu, l.rand_gpu, size, l.c, l.h, l.w, l.out_h, l.out_w, net.train, l.flip, radians, l.output_gpu);
    check_error(cudaPeekAtLastError());

/*
       cuda_pull_array(layer.output_gpu, l.output, size);
       image im = float_to_image(layer.crop_width, l.crop_height, l.c, l.output + 0*(size/layer.batch));
       image im2 = float_to_image(layer.crop_width, l.crop_height, l.c, l.output + 1*(size/layer.batch));
       image im3 = float_to_image(layer.crop_width, l.crop_height, l.c, l.output + 2*(size/layer.batch));
